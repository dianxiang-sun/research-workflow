...
User wants to DESIGN evaluation:
  ...
  → AFTER: Run /research gate 3 (mandatory 12 questions)
...
User wants to REVIEW/AUDIT a paper:
  → /research review (or /research gate)
...
User wants to IMPLEMENT (code, build system):
  ...
  → AFTER: Code freeze tag + dependency lock + /research gate 4
...
|-------------------|----------------|
| No `.claude/research-project.local.md` exists | `/research init` first |
| All Phase 6 sections checked off in progress | `/research review` ("论文写完了，要模拟评审吗？") |
| Gate returns BLOCK + Phase gap > 2 | `/research rollback` ("Phase 5 发现 Phase 2 的问题，要回退吗？") |
| Paper just accepted (user mentions acceptance) | `/research present` + `/research mine-patterns` |
| Paper rejected (user mentions rejection) | `/research rebuttal` (venue pivot analysis) |
| Cost tracker shows >80% budget burned | `/research cost` + scope cut suggestion |
| Phase active >14 days, <50% items done | Nudge: "Phase N 进展缓慢，要 /research status 看看？" |
...
| Same mode used 3+ times with avg score <3 | Suggest: `/research evolve` ("这个功能效果不太好，要优化吗？") |
